<div align="left">
  <a href="./README.md"><img src="https://img.shields.io/badge/lang-en-lightgrey.svg" alt="English"></a>
  <a href="./README_zh-TW.md"><img src="https://img.shields.io/badge/lang-zh--TW-blue.svg" alt="Traditional Chinese"></a>
</div>

## 醫療語意搜尋系統

重要說明：此公開儲存庫為一個乾淨、開源的概念驗證 (proof-of-concept, PoC)，旨在展示一個端到端的搜尋流程，包含查詢重寫 (query rewriting)、密集檢索 (dense retrieval) 及重排序 (reranking)，並僅使用公開可用的資料集。此專案為獨立開發，作為研究用途的參考架構；它不適用於臨床用途，且不包含任何雇主的程式碼、設定或資料。
...
1. 查詢重寫（可選，`ENHANCE_QUERY` 控制）：透過 Ollama 的 `/api/chat` 端點改寫使用者查詢，以提升語意檢索效果。例如，一個簡單的查詢「搜尋目前有 HIV 腦病變的個案」，會被轉換成一個完整的、專家級的查詢：「腦部 MRI 報告，需符合 HIV 腦病變或 HIV 陽性患者的伺機性感染，包括白質高訊號、基底核病灶、皮質萎縮等發現，或存在弓漿蟲症（Toxoplasmosis）、隱球菌瘤（Cryptococcoma）、或進行性多灶性白質腦病（PML）的證據。」
...
命令列工具（CLI）會輸出各流程的評估指標。在開發過程中，我們曾比較數個重排序模型（包含 `BAAI/bge-reranker-v2-gemma`、`ncbi/MedCPT-Cross-Encoder` 與 `Qwen/Qwen3-Reranker-8B`）。此專案預設採用 `BAAI/bge-reranker-v2-gemma`，因其在我們的內部測試中，整體表現最佳。
...
註：
  - 以上是在完整資料集上執行的結果，數值為百分比（%），顯示至小數點後兩位。
  - nDCG@10： 評估前 10 名結果的「排序品質」；最相關的項目排得越前，分數越高。
  - P@10： 評估前 10 名結果的「準確度」；計算這 10 個結果中有多少比例是相關的。
  - R@100： 評估前 100 名結果的「涵蓋率」；衡量系統在所有正確答案中，成功找到了多少比例。
  - MAP： 綜合評估「準確度」與「排序品質」的系統整體平均分數。
...
- **查詢重寫影響**：
  - 查詢重寫能有效增加搜尋廣度，提升 R@100 與 MAP，但同時也引入了噪音，降低 Dense Search 階段頂端的排序精度 (nDCG@10、P@10)。
- **限制與可能因素**：
  - 公開測試集的查詢（專家長句）與真實臨床查詢（簡短、模糊）存在差異，因此量化指標未能完全反應「查詢重寫」在實務上的效益。
...
- **實現 RRF (Reciprocal Rank Fusion) 混合排名**：
  - 開發 RRF 融合機制，將來自多個來源（如：Dense Search、BM25，以及其查詢重寫版本）的排序結果，合併為單一、更穩健的排名。
